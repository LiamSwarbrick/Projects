...
init()
{
    ...
    // Define Chunk
    {
        init_chunk(&chunk0, 0, 0, 0);
        chunk0.blocks[BLOCK_INDEX(0,0,0)] = BLOCK_grass;
        for (int local_x = 0; local_x < CHUNK_WIDTH; ++local_x)
        {
            for (int local_y = 0; local_y < CHUNK_HEIGHT; ++local_y)
            {
                ...
                {
                    chunk0.blocks[BLOCK_INDEX(local_x, local_y, local_z)] = BLOCK_grass;
                }
            }
        }
        update_chunk(&chunk0);
        // Block_Definition block = block_defs[BLOCK_grass];
...
update_and_render()
{
    ...
    // Rendering
    {
        ...
        glDrawElements(GL_TRIANGLES, chunk0.index_counter, GL_UNSIGNED_INT, 0);
    }
}
